/**
 * Fungsi umum untuk penggunaan umum
 * bot. Tidak perlu
 * memodifikasi
 * file ini.
 *
...
exports.loadCommonFunctions = ({ socket, webMessage }) => {
  ...
  const withRetry = async (fn, maxRetries = 3, delayMs = 1000) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        console.warn(
          `Percobaan ${attempt}/${maxRetries} gagal:`,
          error.message
        );
    ...
    throw new Error(
      `Gagal setelah ${maxRetries} percobaan. Kesalahan terakhir: ${lastError.message}`
    );
  };
  ...
  const sendWaitReply = async (text, mentions) => {
    await sendWaitReact();
    return await sendReply(`⏳ Tunggu! ${text || "Memuat data..."}`, mentions);
  };

  const sendWarningReply = async (text, mentions) => {
    await sendWarningReact();
    return await sendReply(`⚠️ Perhatian! ${text}`, mentions);
  };

  const sendErrorReply = async (text, mentions) => {
    await sendErrorReact();
    return await sendReply(`❌ Kesalahan! ${text}`, mentions);
  };
