    ...
    try {
      const targetCommand = commands.find((cmd) => {
        if (!cmd?.commands || !Array.isArray(cmd.commands)) {
          errorLog(
            `Error dalam perintah tipe "${type}": Properti "commands" harus ada dan berupa ["array"] dengan nama-nama perintah! File bermasalah: ${cmd.name}.js`
          );
      ...
      console.error(`Error saat memproses perintah tipe "${type}":`, error);
    }
  }
    ...
    const files = readDirectoryRecursive(subdirectoryPath)
      .map((filePath) => {
        try {
          ...
          console.error(`Error saat mengimpor ${filePath}:`, err);
          return null;
        }
      })
      .filter(Boolean);
...
exports.getImageBuffer = async (url, options = {}) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Gagal mendapatkan gambar: ${response.status} ${response.statusText}`
      );
    }
    ...
    errorLog(`Error saat mendapatkan buffer gambar: ${error.message}`);
    throw error;
  }
};
