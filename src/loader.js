/**
 * Script ini bertanggung jawab
 * untuk memuat event
 * yang akan didengarkan oleh
 * socket WhatsApp.
 *
 * @author Dev Gui
 */
...
exports.load = (socket) => {
  ...
  const safeEventHandler = async (callback, data, eventName) => {
    try {
      ...
      errorLog(`Error saat memproses event ${eventName}: ${error.message}`);
  ...
  process.on("uncaughtException", (error) => {
    ...
    errorLog(`Error yang tidak tertangani: ${error.message}`);
  });
